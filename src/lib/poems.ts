...
export function adjacentPoems(slug: string): { prev: Poem | null; next: Poem | null } {
...
export const yearSections: { year: string; poems: Poem[] }[] = (() => {
  const order: string[] = [];
  ...
  return order.map((year) => ({ year, poems: map.get(year)! }));
})();
